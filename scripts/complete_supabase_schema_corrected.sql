...
CREATE POLICY "Users can view their own user entry." ON public.users
  FOR SELECT USING (auth.uid() = id);

CREATE POLICY "Users can insert their own user entry." ON public.users
  FOR INSERT WITH CHECK (auth.uid() = id);

CREATE POLICY "Users can update their own user entry." ON public.users
  FOR UPDATE USING (auth.uid() = id);

CREATE POLICY "Users can delete their own user entry." ON public.users
...
-- Corrected banking_accounts table: Ensure balance is not nullable
ALTER TABLE public.banking ALTER COLUMN balance SET NOT NULL;
...
-- Corrected expenses table: Ensure amount is not nullable
...
-- Corrected products table: Ensure price and stock_quantity are not nullable
ALTER TABLE public.inventory ALTER COLUMN price SET NOT NULL;
ALTER TABLE public.inventory ALTER COLUMN stock SET NOT NULL;
...
-- Corrected purchases table: Ensure quantity and total_amount are not nullable
...
-- Corrected sales table: Ensure quantity and total_amount are not nullable
ALTER TABLE public.sales ALTER COLUMN quantity SET NOT NULL;
ALTER TABLE public.sales ALTER COLUMN total_amount SET NOT NULL;

-- Add foreign key for product_id in sales table
ALTER TABLE public.sales
ADD CONSTRAINT sales_product_id_fkey
FOREIGN KEY (product_id) REFERENCES public.inventory(id) ON DELETE CASCADE;

-- Add foreign key for client_id in sales table
ALTER TABLE public.sales
