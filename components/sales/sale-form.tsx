...
import type { Sale, Client, Product } from "@/lib/supabase/types"
...
export function SaleForm({ initialData, clients, products }: SaleFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (initialData) {
      const result = await updateSale(initialData.id, dataToSubmit)
      error = result?.error
    } else {
    ...
    if (error) {
      ...
      toast({
        title: "Success",
        description: `Sale ${initialData ? "updated" : "created"} successfully.`,
      })
      router.push("/sales")
    }
  }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        />
        <Button type="submit">{initialData ? "Update" : "Create"} Sale</Button>
      </form>
    </Form>
  )
}
