...
CREATE TABLE sales (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    product_id UUID NOT NULL REFERENCES products(id) ON DELETE RESTRICT, -- RESTRICT to prevent deleting products with existing sales
...
CREATE TABLE purchases (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    product_id UUID NOT NULL REFERENCES products(id) ON DELETE RESTRICT, -- RESTRICT to prevent deleting products with existing purchases
...
CREATE POLICY "Allow read access to sale_items for authenticated users" ON sale_items FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Allow insert access to sale_items for authenticated users" ON sale_items FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Allow update access to sale_items for authenticated users" ON sale_items FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Allow delete access to sale_items for authenticated users" ON sale_items FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Allow read access to purchase_items for authenticated users" ON purchase_items FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Allow insert access to purchase_items for authenticated users" ON purchase_items FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Allow update access to purchase_items for authenticated users" ON purchase_items FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Allow delete access to purchase_items for authenticated users" ON purchase_items FOR DELETE USING (auth.uid() = user_id);
