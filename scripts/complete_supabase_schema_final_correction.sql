...
CREATE POLICY "Authenticated users can delete banking transactions." ON banking_transactions FOR DELETE USING (auth.uid() = id); -- Corrected policy: users can only delete their own transactions
